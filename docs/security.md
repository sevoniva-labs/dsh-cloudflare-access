...
Cloudflare 撤权、浏览器后台定时器限速、网络切换可能导致重连。租约是有界重验，不是即时全局撤权承诺。租约仍绑定完整 JWT 的指纹，不按邮箱续旧连接；浏览器收到新的不透明会话标记时，通过官方 Connection 重建连接。

客户端只重试租约探测，不重放任务、审批或模型请求。401/认证跳转进入认证恢复；403 保持拒绝；网络失败和 5xx 不冒充登录过期。凭据年龄上限可配置，默认两小时；HTTP 校验与已打开连接使用同一个有效截止时间。

静默认证只导航到同源、受 Access 保护的固定回调路径，不保存或转发 Access Cookie/JWT。iframe 回调校验 origin、source 和一次性随机 state；交互登录窗口使用 noopener，通过同源 BroadcastChannel 通知。通知不授予权限，客户端必须再次通过网关校验。回调不接受外部跳转地址，不返回个人身份或凭据，使用 nonce CSP、同源 frame-ancestors 与 no-store。

静默认证每次失效周期只尝试一次。第三方 Cookie/嵌入限制或全局登录到期时，用户需在新窗口完成正常 Cloudflare 认证；不绕过 Access，不自动退出登录，不无限刷新页面。切换账号时停止自动恢复，提示用户保存草稿并重新打开页面。
